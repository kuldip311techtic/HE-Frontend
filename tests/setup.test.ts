...
describe('admin routes', () => {
  ...
  it('registers manage users in admin navigation', () => {
    expect(paths.manageUsers).toBe('/super-admin/manage-users');
    expect(adminNavigation.map((item) => item.to)).toContain(paths.manageUsers);
  });
});
