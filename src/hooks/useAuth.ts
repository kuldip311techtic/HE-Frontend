...
import type { ApiErrorEnvelope, ErrorResponse, LoginRequest, LoginResponse } from '../types/api';
...
export function getAuthErrorMessage(error: unknown): string {
  if (axios.isAxiosError<ErrorResponse | ApiErrorEnvelope>(error)) {
    ...
    if (data && 'detail' in data && typeof data.detail === 'string' && data.detail) {
      return data.detail;
    }

    if (data && 'message' in data && typeof data.message === 'string' && data.message) {
      return data.message;
    }
